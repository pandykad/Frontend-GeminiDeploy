...
function getHandleLookup({ nodeLookup, nodeId, handleId, handleType }) {
  const connectionHandles = /* @__PURE__ */ new Map();
  ...
  if (!matchingHandle) {
    const node = nodeLookup.get(nodeId);
    if (node == null ? void 0 : node.internals.handleBounds) {
...
function isValidHandle(event, { handle, connectionMode, fromNodeId, fromHandleId, fromType, doc, lib, flowId, isValidConnection = alwaysValid, handleLookup }) {
  ...
  if (handleToCheck) {
    ...
    const toHandle = handleLookup == null ? void 0 : handleLookup.get(`${handleNodeId}-${handleType}-${handleId}`);
    if (toHandle) {
      result.toHandle = {
        ...toHandle
      };
...
var selector$p = (s) => s.ariaLiveMessage;
...
var selector$o = (s) => s.userSelectionActive ? "none" : "all";
...
var selector$e = (onlyRenderVisible) => (s) => {
...
function useVisibleNodeIds(onlyRenderVisible) {
  const nodeIds = useStore((0, import_react2.useCallback)(selector$e(onlyRenderVisible), [onlyRenderVisible]), shallow$1);
  return nodeIds;
}
var selector$d = (s) => s.updateNodeInternals;
function useResizeObserver() {
  const updateNodeInternals2 = useStore(selector$d);
...
function NodeWrapper({ id, onClick, onMouseEnter, onMouseMove, onMouseLeave, onContextMenu, onDoubleClick, nodesDraggable, elementsSelectable, nodesConnectable, nodesFocusable, resizeObserver, noDragClassName, noPanClassName, disableKeyboardA11y, rfId, nodeTypes, nodeExtent, nodeClickDistance, onError }) {
  ...
  const clampedPosition = nodeExtent ? clampPosition(internals.positionAbsolute, nodeExtent) : internals.positionAbsolute;
...
var selector$a = (s) => `translate(${s.transform[0]}px,${s.transform[1]}px) scale(${s.transform[2]})`;
...
var getInitialState = ({ nodes, edges, defaultNodes, defaultEdges, width, height, fitView: fitView2, nodeOrigin } = {}) => {
...
var createStore2 = ({ nodes, edges, defaultNodes, defaultEdges, width, height, fitView: fitView$1, nodeOrigin }) => createWithEqualityFn((set, get) => ({
  ...
  setNodes: (nodes2) => {
    ...
    adoptUserNodes(nodes2, nodeLookup, parentLookup, { nodeOrigin: nodeOrigin2, elevateNodesOnSelect, checkEquality: true });
    set({ nodes: nodes2 });
  },
  setEdges: (edges2) => {
  ...
  updateNodeInternals: (updates) => {
    const { triggerNodeChanges, nodeLookup, parentLookup, fitViewOnInit, fitViewDone, fitViewOnInitOptions, domNode, nodeOrigin: nodeOrigin2, debug, fitViewSync } = get();
    const { changes, updatedInternals } = updateNodeInternals(updates, nodeLookup, parentLookup, domNode, nodeOrigin2);
    if (!updatedInternals) {
      return;
    }
...
function ReactFlowProvider({ initialNodes: nodes, initialEdges: edges, defaultNodes, defaultEdges, initialWidth: width, initialHeight: height, fitView: fitView2, nodeOrigin, children }) {
...
function Wrapper({ children, nodes, edges, defaultNodes, defaultEdges, width, height, fitView: fitView2, nodeOrigin }) {
  const isWrapped = (0, import_react2.useContext)(StoreContext);
  if (isWrapped) {
    return (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, { children });
  }
  return (0, import_jsx_runtime.jsx)(ReactFlowProvider, { initialNodes: nodes, initialEdges: edges, defaultNodes, defaultEdges, initialWidth: width, initialHeight: height, fitView: fitView2, nodeOrigin, children });
}
var wrapperStyle = {
  width: "100%",
  height: "100%",
